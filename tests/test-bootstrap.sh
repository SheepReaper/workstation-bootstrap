...
grep -q 'proc/sys/kernel/osrelease' "$root/bootstrap.sh"
grep -q 'not as root' "$root/bootstrap.sh"
grep -q 'npiperelay.exe is not visible' "$root/bootstrap.sh"
grep -q 'wsl.exe --shutdown' "$root/bootstrap.sh"
